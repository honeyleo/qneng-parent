...
var jqWindowsEngineZIndex = 9999;
(function($){ 
...
$.fn.newWindow = function(options){
    ...
    $windowdivWidth = $('<div class="window-divWidth"></div>');
	...
	var resize = function($obj, width, height,$obj2){
	    
		width = parseInt(width);
		height = parseInt(height);
        $obj2.text(width +'×'+ height);
	...
	var resizing = function(e, $obj,$obj2){
	...
	$windowResizeIcon.bind('mousedown', function(e){
        ...
        $obj2 = $(e.target).parent().parent().children(":first");
		setFocus($obj);
	...
	$windowMaximizeButton.bind('click', function(e){
	  ...
	  if($obj.attr("state") == "normal"){
		  if(options.windowType=="standard"){
			...
			move($obj, 5, 5);
		    resize($obj,$(window).width()-15,$(window).height()-45,$obj2);
			$obj.attr("lastX",tmpx);
		    $obj.attr("lastY",tmpy);
	    ...
	    if(options.windowType=="standard"){ 
		  ...
		  resize($obj,$obj.attr("lastWidth"),$obj.attr("lastHeight"),$obj2);
	...
	resize($windowContainer,options.width,options.height,$windowdivWidth);
    ...
    $windowContainer.append($windowdivWidth)
